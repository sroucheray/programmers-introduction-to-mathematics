import { Polynomial, ZERO } from './polynomial.js';
/**
 ...
 */
export const singleTerm = (points, i) => {
    ...
    for (let j = 0; j < points.length; j++) {
        ...
        //                         +   (1 / (xi - xj)) * t^1
        theTerm = theTerm.multiply(new Polynomial([-xj / (xi - xj), 1 / (xi - xj)]));
 ...
 */
export const interpolate = (points) => {
    if (points.length === 0) {
    ...
    return Array.from({ length: points.length }, (_, index) => singleTerm(points, index)).reduce((result, term) => {
